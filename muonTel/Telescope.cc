...
//
/// \file exampleEX1.cc
/// \brief Main program of the EX1 example
...
// ======= RUN MANAGER ==========
	...
	//----- Step Zero: Construct the run manager----------
	//run manager takes care of all the actions
  G4RunManager* runManager = new G4RunManager;

	//----- Step One: Construct the detector----------
  auto detConstruction = new DetectorConstruction();
  runManager->SetUserInitialization(detConstruction);

	//----- Step Two: Construct the physics list----------  
  auto physicsList = new FTFP_BERT_HP;  
  physicsList->RegisterPhysics(new G4OpticalPhysics());	// Optical module
  physicsList->SetVerboseLevel(1);  
  runManager->SetUserInitialization(physicsList);

  
	//---- Step Three: Construct the Actions -------	
	runManager->SetUserInitialization(new ActionInitialization(detConstruction));

//---- Step Four: Initialize the run manager -------
...
/*  
  // Optional, but sometimes useful for debugging
...
*/  
  ...
  if ( ! ui ) {
    ...
    // interactive mode
    runManager->Initialize();  // AGGIUNGI QUESTA RIGA!
